...
def main():
    ...
    logger, final_output_dir, tb_log_dir = create_logger(config, 'test')
    ...
    log_info(f'{len(test_dataset)} validation examples in {len(testloader)} batches')

    start_time = time.time()

    log_info('Testing ...')
    scores = function.test(model, testloader, test_dataset, config, config.METRIC.METRICS, sv_dir=config.TEST.SV_DIR)
    for metric in scores.keys():
        message = '{}: {}, Per Class {}:{} '.format(
            metric,
            scores[metric][0],
            metric,
            scores[metric][1:]
    ...
    log_info(f'Testing time {total_time_str}')
